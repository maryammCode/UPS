        ...
        <select class="form-control select2" name="{{ $row->field }}">
            ...
            @if (isset($options->options))
                <optgroup label="{{ __('voyager::generic.custom') }}">
                    @foreach ($options->options as $key => $option)
                        <option value="{{ $key == '_empty_' ? '' : $key }}"
                            @if ($default == $key && $selected_value === null) selected="selected" @endif
            ...
            if (method_exists($dataTypeContent, $relationshipListMethod)) {
                ...
                if (isset($options->relationship->where)) {
                    $relationshipOptions = $relationshipClass::where($options->relationship->where[0], $options->relationship->where[1])->get();
                } else {
            ...
            <optgroup label="{{ __('voyager::database.relationship.relationship') }}">
                @foreach ($relationshipOptions as $relationshipOption)
                    <option value="{{ $relationshipOption->{$options->relationship->key} }}"
                        @if ($default == $relationshipOption->{$options->relationship->key} && $selected_value === null) selected="selected" @endif
                        @if ($selected_value == $relationshipOption->{$options->relationship->key}) selected="selected" @endif>
                        {{ $relationshipOption->{$options->relationship->label} }}</option>
                @endforeach
            </optgroup>
        </select>
    @else
...
{{-- @php
    ...
    foreach ($routeCollection as $route) {
        if (in_array('GET', $route->methods()) && $route->getName()) {
            $getRoutes[] = $route->getName();
        }
    }
    dd($getRoutes);
@endphp --}}
    {{-- start add by ahmed --}}
    @if (in_array($dataType->slug, ['home-shortcuts']))
        {{-- Debugging output for routeNames --}}
        {{-- @dd($dataType) --}}
        @if (isset($dataType->routeNames))
            ...
            <select class="form-control select2" name="{{ $row->field }}">
                @foreach ($dataType->routeNames as $routeName)
