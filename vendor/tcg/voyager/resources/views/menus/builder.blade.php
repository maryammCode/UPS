        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="" id="m_form" method="POST" data-action-add="{{ route('voyager.menus.item.add', ['menu' => $menu->id]) }}"
                    data-action-update="{{ route('voyager.menus.item.update', ['menu' => $menu->id]) }}" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        {{-- Mega Menu --}}
                        <label for="type">{{ __('voyager::menu_builder.mega_menu') }}</label>
                        <select id="m_mega_menu" class="form-control" name="mega_menu">
                            <option value="1" selected="selected">{{ __('voyager::menu_builder.Yes') }}</option>
                            <option value="0">{{ __('voyager::menu_builder.No') }}</option>
                        </select>
                        <br>
                        {{-- Mega Menu --}}
                        ...
                        <br>
                        {{-- Display Type --}}
                        <label for="m_display">{{ __('voyager::menu_builder.display_type') }}</label>
                        <select id="m_display" class="form-control" name="display">
                            <option value="list" selected="selected">{{ __('voyager::menu_builder.List') }}</option>
                            <option value="image">{{ __('voyager::menu_builder.Image') }}</option>
                        </select>
                        <br>
                        {{-- Display Type --}}
                        {{-- Breadcrumb File --}}
                        <label for="m_breadcrumb">{{ __('voyager::menu_builder.Breadcrumb') }}</label>
                        <input type="file" class="form-control" id="m_breadcrumb" name="breadcrumb"
                            style="padding: 6px" autofocus
                            accept="image/png, image/jpeg, image/webp, image/jpg, .pdf"><br>
                        <br>
                        <!-- Display Breadcrumb Image -->
                        {{-- <img id="m_breadcrumb_preview" src="" alt="Breadcrumb Image" style="max-width: 100%; display: none;"> --}}
                        <div id="m_breadcrumb_preview_container" style="display: none;">
                            <a href="#" class="voyager-x remove-single-image" style="position: absolute;"
                                id="delete_image_btn"></a>
                            <img src="" id="m_breadcrumb_preview"
                                style="max-width: 100%; height: auto; clear: both; display: none; padding: 2px; border: 1px solid #ddd; margin-bottom: 10px;">
                        </div>
                        {{-- Breadcrumb File --}}
    ...
    <script>
        $(document).ready(function () {
            ...
            var $m_modal       = $('#menu_item_modal'),
                ...
                $m_color       = $('#m_color'),
                $m_target      = $('#m_target'),
                $m_mega_menu = $('#m_mega_menu'),
                $m_display = $('#m_display'),
                $m_breadcrumb = $('#m_breadcrumb'),
                $m_breadcrumb_preview = $('#m_breadcrumb_preview'),
                $m_id          = $('#m_id');
            ...
            $m_modal.on('show.bs.modal', function(e, data) {
                ...
                if (_adding) {
                    ...
                    $m_mega_menu.val('1').change();
                    $m_display.val('list').change();
                    ...
                    $m_mega_menu.val(_src.data('mega_menu'));
                    $m_display.val(_src.data('display'));
                    ...
                    // Mega Menu Start
                    if (_src.data('mega_menu') == '1') {
                        $m_mega_menu.val('1').change();
                    } else if (_src.data('mega_menu') == '0') {
                        $m_mega_menu.find("option[value='1']").removeAttr('selected');
                        $m_mega_menu.find("option[value='0']").attr('selected', 'selected');
                        $m_mega_menu.val('0');
                    }
                    // Mega Menu End

                    // Display Type Start
                    if (_src.data('display') === 'list') {
                        $m_display.val('list').change();
                    } else if (_src.data('display') === 'image') {
                        $m_display.find("option[value='list']").removeAttr('selected');
                        $m_display.find("option[value='image']").attr('selected', 'selected');
                        $m_display.val('image');
                    }
                    // Display Type End

                    if (_src.data('breadcrumb')) {
                        $('#m_breadcrumb_preview').attr('src', _src.data('breadcrumb')).show();
                        $('#m_breadcrumb_preview_container').show();
                    } else {
                        $('#m_breadcrumb_preview').hide();
                        $('#m_breadcrumb_preview_container').hide();
                    }
             ...
             */
            $('#m_breadcrumb').change(function() {
                var reader = new FileReader();
                reader.onload = function(e) {
                    $('#m_breadcrumb_preview').attr('src', e.target.result).show();
                };
