...
function hasBgRendering(def) {
    return (
        def.rendering === "background" || def.rendering === "inverse-background"
    );
}
...
function isNewPropsValid(newProps, calendar) {
    ...
    return (calendar.pluginSystem.hooks.isPropsValid || isPropsValid)(
        props,
        calendar
    );
}
function isPropsValid(state, calendar, dateSpanMeta, filterConfig) {
    ...
    if (
        state.eventDrag &&
        !isInteractionPropsValid(state, calendar, dateSpanMeta, filterConfig)
    ) {
        return false;
    }
    if (
        state.dateSelection &&
        !isDateSelectionPropsValid(state, calendar, dateSpanMeta, filterConfig)
    ) {
        return false;
    }
    return true;
...
function processUnscopedUiProps(rawProps, calendar, leftovers) {
    ...
    return {
        startEditable:
            props.startEditable != null ? props.startEditable : props.editable,
        durationEditable:
...
function getAllDayHtml(component) {
    return (
        component.opt("allDayHtml") || htmlEscape(component.opt("allDayText"))
    );
}
...
function fetchSource(eventSource, fetchRange, calendar) {
    var sourceDef =
        calendar.pluginSystem.hooks.eventSourceDefs[eventSource.sourceDefId];
...
function reduceDateProfile(
    ...
    switch (action.type) {
        case "PREV":
            newDateProfile = calendar.dateProfileGenerators[viewType].buildPrev(
                currentDateProfile,
                currentDate
            );
            break;
        case "NEXT":
            newDateProfile = calendar.dateProfileGenerators[viewType].buildNext(
                currentDateProfile,
                currentDate
            );
            break;
...
var CalendarComponent = /** @class */ (function (_super) {
    ...
    function CalendarComponent(context, el) {
        ...
        for (
            var _i = 0, _a = calendar.pluginSystem.hooks.viewContainerModifiers;
            _i < _a.length;
            _i++
        ) {
...
var Calendar = /** @class */ (function () {
    ...
    */
    ...
    Calendar.prototype.publiclyTriggerAfterSizing = function (name, args) {
        ...
        (afterSizingTriggers[name] || (afterSizingTriggers[name] = [])).push(
            args
        );
    };
